...
export default function BusScheduleDriver(props) {
  ...
  const handleSearchCustomer = useCallback(() => {
    let cloneData = [...data];
    let filterData = cloneData.filter(
      (e) => e.id.toString().includes(search) || e.passenger_phone.includes(search) || e.seat.includes(search)
    );
    setData(filterData);
  }, [search, data]);
  ...
  const handleChangeQuerySearch = useCallback((value) => {
    if (value == '') {
      setData(props.data);
    }
    setSearch(value);
          ...
          <Flex padding="2% 4%" alignItems={'center'}>
            ...
            {customer.payment_status != 2 && (
              <IconButton
                ...
                w="70px"
                h="70px"
                onClick={() => handleCheckCustomerInTheCar(customer.id, customer.payment_status)}
              />
            )}
          </Flex>
        </Box>
