	...
	const updateUserInformation = useCallback(
		async (status) => {
			...
			if (transactionId) {
				...
				try {
					...
					if (err.response.data.statusCode == 401) {
						...
						toastIdRef.current = toast({
							title: err.response.data.data.message,
							description:
								"Có lỗi xảy ra khi cập nhật thông tin khách hàng. Vui lòng thử lại",
							status: "error",
							isClosable: true,
							position: "top",
							duration: 2000,
				...
				submitData.created_by = state.dataUser.id;
				if (paymentStatus == 1) {
					submitData.cashier = state.dataUser.id;
				} else if (paymentStatus == 0) {
					submitData.cashier = 0;
				}
				submitData.bankCode = "";
				...
				<ModalBody>
					...
					>
						...
						<Box>
							<FormControl
								marginBottom="5%"
								isInvalid={error.pickupLocation}
							>
								<Flex>
									<FormLabel
										width={"51.5%"}
