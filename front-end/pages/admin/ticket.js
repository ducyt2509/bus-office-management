...
export default function Ticket(props) {
	...
	const cancelTicket = useCallback(async () => {
		...
		cloneSeatCustomerSelected = cloneSeatCustomerSelected.filter(
			(s) => !seatSelected.includes(s),
		);
	...
	useEffect(() => {
		...
		if (token && JSON.parse(userData).role_id == 1) {
			handleTotalRenewal();
		}
	}, [token]);
			...
			<ModalContent>
				<ModalBody textAlign={"center"}>
					...
					<Text
						...
						color={"#363636"}
					>
						Sau khi xe đã xuất bến, hành khách và nhân viên không thể đặt hay chỉnh sửa vé
						được nữa. Bạn có chắc chắn muốn xuất bến?
					</Text>
					<Flex
