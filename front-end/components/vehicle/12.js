...
export default function Seat12(props) {
  ...
  const ChairHTML = (id) => {
    return (
      <Image
        src={props.seatSelected.includes(id) ? SelectChair : EmptyChair}
  ...
  const SoldChairHTML = (
    <Image src={SoldChair} style={{ cursor: 'not-allowed', width: '43%', height: '100%' }} />
  );
  const SelectChairHTML = (
    <Image src={SelectChair} style={{ cursor: 'pointer', width: '43%', height: '100%' }} />
  );
  const SteeringHTML = <Image src={Steering} style={{ cursor: 'pointer', width: '25%' }} />;
  return (
    <Stack className="bom-vehicle-type" justifyContent={'space-evenly'}>
