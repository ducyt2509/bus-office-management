...
module.exports = {
	...
	async addNewTransport(req, res) {
		try {
			const { bus_schedule_id,
				bus_id,
				departure_date } = req.body;
			if (!validateHandler.validatePositiveIntegerNumber(bus_schedule_id) ||
				!validateHandler.validatePositiveIntegerNumber(bus_id) ||
				!validateHandler.validateDate(departure_date))
				return responseHandler.badRequest(res, messageHandler.messageValidateFailed)
			const isExist = await checkExistTransport(bus_schedule_id,
				bus_id,
				departure_date)
